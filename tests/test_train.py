...
def _make_temp_data(tmp_path):
    ...
    # 2.10.1: Tạo mảng X có kích thước (n, len(FEATURE_NAMES))
    X = rng.random((n, len(FEATURE_NAMES)))
    
    # 2.10.2: Tạo mảng y có n phần tử, mỗi phần tử là số nguyên trong [0, 3)
    y = rng.integers(0, 3, size=n)
    
    # 2.10.3: Tạo DataFrame từ X và y
    df = pd.DataFrame(X, columns=FEATURE_NAMES)
    df["target"] = y
    
    # 2.10.4: Lưu train và eval
...
def test_metrics_file_created(tmp_path):
    ...
    # 2.10.8: assert file tồn tại
    assert os.path.exists("outputs/metrics.json")
    
    # 2.10.9: assert nội dung
